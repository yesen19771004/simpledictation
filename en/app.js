  ...
  if (clearBtn) {
    clearBtn.addEventListener('click', () => {
      if (confirm('确定清空所有数据吗？\n\n这将删除：\n· 所有听写记录\n· 所有错误记录\n· 所有资料库导入映射\n\n此操作不可恢复，请确认已导出备份。')) {
        ...
        try { localStorage.removeItem(LANG_PREFIX + 'dictation-voice-region'); } catch {}
        try { localStorage.removeItem(LANG_PREFIX + 'dictation-drill-level'); } catch {}
        showToast('数据已清空');
        renderMain();
      }
    });
...
function getSavedVoiceRegion() {
  try { return localStorage.getItem(LANG_PREFIX + 'dictation-voice-region') || 'US'; } catch { return 'US'; }
}
...
function attachVoiceToggleListeners(container) {
  ...
  toggle.addEventListener('click', (e) => {
    ...
    if (voice) {
      selectedVoice = voice;
      try { localStorage.setItem(LANG_PREFIX + 'dictation-voice-region', region); } catch {}
...
function speak(text, rate = 1.0, onEnd) {
  ...
  // 每次 speak 时重新查找 Voice，避免 Chrome 中缓存的 Voice 对象引用失效
