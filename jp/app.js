...
function renderPractice(container) {
  ...
  card.innerHTML = `
    ...
    <div class="side-panel" id="side-panel" style="display:none">
      <div id="sentence-original" style="padding:16px 24px;font-size:16px;font-weight:600;color:var(--text);line-height:1.6"></div>
    </div>
  `;
  container.appendChild(card);
  ...
  card.querySelector('#btn-show-original').addEventListener('click', () => {
    ...
    if (showing && !sidePanel.dataset.loaded) {
      ...
      const origEl = card.querySelector('#sentence-original');
      if (origEl) origEl.textContent = sentence;
    }
  });
